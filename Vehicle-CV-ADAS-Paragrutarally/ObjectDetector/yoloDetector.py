...
try :
	...
	sys.path.append("..")
	# Updated import for Jetson compatibility
	from coreEngine import create_engine, is_jetson
except :
	from .utils import ObjectModelType, hex_to_rgb, NMS, Scaler
	from .core import ObjectDetectBase, RectInfo
	# Updated import for Jetson compatibility
...
class YoloDetector(ObjectDetectBase, YoloLiteParameters):
	...
	def _initialize_model(self, model_path : str) -> None:
		"""Initialize model with Jetson compatibility"""
		model_path = os.path.expanduser(model_path)
		if (self.logger) :
			self.logger.debug("model path: %s." % model_path)

		try:
			# Use the factory function for better compatibility
			self.engine = create_engine(model_path)
			
			if (self.logger) :
				platform_info = "Jetson" if is_jetson() else "Standard"
				self.logger.info(f'YoloDetector [{platform_info}] Type: [{self.engine.framework_type}] || Version: [{self.engine.providers}]')
			
		except Exception as e:
			if self.logger:
				self.logger.error(f"Failed to initialize model {model_path}: {e}")
			
			# Try fallback to ONNX if TensorRT fails
			if model_path.endswith('.trt'):
				onnx_path = model_path.replace('.trt', '.onnx')
				if os.path.exists(onnx_path):
					if self.logger:
						self.logger.info(f"Falling back to ONNX model: {onnx_path}")
					try:
						self.engine = create_engine(onnx_path)
					except Exception as e2:
						raise Exception(f"Both TensorRT and ONNX initialization failed. TRT: {e}, ONNX: {e2}")
				else:
					raise Exception(f"TensorRT initialization failed and no ONNX fallback found: {e}")
			else:
				raise e
...
if __name__ == "__main__":
	...
	config = {
		"model_path": 'models/yolov5n-coco_fp16.onnx',  # Use FP16 for Jetson
		"model_type" : ObjectModelType.YOLOV5,
		"classes_path" : 'models/coco_label.txt',
		"box_score" : 0.4,
		"box_nms_iou" : 0.45,
