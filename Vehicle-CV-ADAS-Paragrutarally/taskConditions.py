...
class TaskConditions(object):
    ...
    def UpdateRoadStatus(self, is_on_road, road_center=None, vehicle_position=None):
        ...
        Args:
            ...
            road_center: Array of road center points
        ...
        if road_center is not None and len(road_center) > 0 and vehicle_position is not None:
            # Find the closest point on the center line to vehicle position
            vehicle_x, vehicle_y = vehicle_position
            # Convert road_center to numpy array if it's not already
            if not isinstance(road_center, np.ndarray):
                road_center = np.array(road_center)
                
            # Calculate distance to each center point
            distances = np.sum((road_center - np.array([vehicle_x, vehicle_y])) ** 2, axis=1)
            if len(distances) > 0:
