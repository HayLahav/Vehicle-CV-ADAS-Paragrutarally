#!/usr/bin/env python3
"""
Enhanced Jetson Orin Nano ADAS Demo with Moondream2 VLM Integration
Combines traditional CV-based ADAS with Vision-Language Model for enhanced scene understanding
"""
...
def optimize_jetson_performance():
    """Optimize Jetson Xavier/Orin performance"""
    if not is_jetson():
        return
...
class ControlPanel(object):
    ...
    def __init__(self, enable_vlm=True):
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='ADAS with VLM on Jetson Orin Nano')
    parser.add_argument('--video', type=str, default='./TrafficLaneDetector/temp/demo-7.mp4',
                       help='Path to input video')
    parser.add_argument('--enable-vlm', action='store_true', default=True,
                       help='Enable Moondream2 VLM integration')
    parser.add_argument('--vlm-interval', type=int, default=60,
                       help='VLM analysis interval in frames (default: 60)')
    parser.add_argument('--output', type=str, default=None,
                       help='Output video path (default: input_vlm_out.mp4)')
...
if __name__ == "__main__":
    ...
    output_path = args.output or video_path[:-4] + '_vlm_out.mp4'
    ...
    cv2.namedWindow("ADAS + VLM - Jetson", cv2.WINDOW_NORMAL)
    ...
    vlmAnalyzer = None
    if args.enable_vlm:
        LOGGER.info("Initializing Moondream2 VLM...")
        try:
            vlmDetector = MoondreamVLMDetector(logger=LOGGER)
            if vlmDetector.is_available():
                vlmAnalyzer = VLMSceneAnalyzer(vlmDetector, logger=LOGGER)
                vlmAnalyzer.set_analysis_interval(args.vlm_interval)
                LOGGER.info(f"VLM initialized successfully (analysis every {args.vlm_interval} frames)")
            else:
                LOGGER.war("VLM initialization failed - continuing without VLM")
                args.enable_vlm = False
        except Exception as e:
            LOGGER.war(f"VLM initialization failed: {e}")
            args.enable_vlm = False

    # Initialize display panel and task condition analyzer
    displayPanel = ControlPanel(enable_vlm=args.enable_vlm)
    ...
    try:
        while cap.isOpened():
            ...
            if ret:
                ...
                #========================== VLM Analysis ==========================
                if args.enable_vlm and vlmAnalyzer:
                    try:
                        vlm_result = vlmAnalyzer.analyze_frame(frame)
                        if vlm_result:
                            LOGGER.info(f"VLM Scene: {vlm_result['scene_description']}")
                    except Exception as e:
                        LOGGER.war(f"VLM analysis failed: {e}")
                ...
                try:
                    ...
                    # Draw VLM info
                    if args.enable_vlm and vlmAnalyzer:
                        vlmAnalyzer.draw_vlm_info(frame_show)

                except Exception as e:
                    LOGGER.war(f"Drawing failed: {e}")

                # Show result
                cv2.imshow("ADAS + VLM - Jetson", frame_show)
        ...
        if args.enable_vlm and vlmDetector:
            avg_vlm_time = vlmDetector.get_avg_inference_time()
            LOGGER.info(f"Average VLM inference time: {avg_vlm_time:.2f}s")
